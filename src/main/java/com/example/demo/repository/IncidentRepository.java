...
public interface IncidentRepository extends JpaRepository<Incident, UUID> {
	...
	List<Incident> findIncidentsInDateRange(@Param("startDate") LocalDateTime startDate,
	...
	List<Incident> findResolvedIncidentsInDateRange(@Param("startDate") LocalDateTime startDate,
			@Param("endDate") LocalDateTime endDate);

	@Query("""
			    SELECT new com.example.demo.dto.response.IncidentCountResponse(
			        COUNT(i),
			        SUM(CASE WHEN i.isActive = true THEN 1 ELSE 0 END),
			        SUM(CASE WHEN i.isActive = false THEN 1 ELSE 0 END)
			    )
			    FROM Incident i
			""")
	IncidentCountResponse getIncidentCount();
	...
	@Query("""
			    ...
			    FROM Incident i where i.village.id = :villageId
			""")
	IncidentCountResponse getIncidentCountByVillageId(UUID villageId);
}
