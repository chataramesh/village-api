...
public interface EventRepository extends JpaRepository<Event, UUID> {
	...
	List<Event> findEventsInTimeRange(@Param("startTime") LocalDateTime startTime,
	...
	@Query("""
			    SELECT new com.example.demo.dto.response.EventCountResponse(
			        COUNT(e),
			        SUM(CASE WHEN e.isActive = true THEN 1 ELSE 0 END),
			        SUM(CASE WHEN e.isActive = false THEN 1 ELSE 0 END)
			    )
			    FROM Event e where e.village.id = :villageId
			""")
	EventCountResponse getEventCountByVillageId(UUID villageId);
	...
	@Query("""
			    ...
			    FROM Event e
			""")
	EventCountResponse getEventCount();
}
